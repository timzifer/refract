...
- The built-in emitter must be deterministic, because golden files compare it
  byte for byte. Attribute order and number formatting are fixed for that
  reason, and `TestOutputIsDeterministic` holds it to it.
