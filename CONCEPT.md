...
- More coordinate systems: geographic and map projections. Polar arrived in v0.8
  ...
  clip changes ([ADR 0050](docs/adr/0050-barycentric-coord.md)). A projection is
...
- Node-link diagrams and Venn/UpSet, which are what is left of the relational
  ...
  **That refusal was one size too large**, and the correction is planned: it
  ...
  ([ADR 0052](docs/adr/0052-tidy-tree-layout.md)). The clustered heatmap that
...
- More stats: contour. Normal QQ plots shipped in v1.5, with a general
  ...
  ([ADR 0053](docs/adr/0053-statistical-instruments.md)).
- A **locus** — a family of curves given by a formula rather than by data, of
  ...
  its two tick lists ([ADR 0049](docs/adr/0049-locus-annotations.md)).
- A **probability scale**, beside `Log` and `SymLog`: an axis warped by
  ...
  ([ADR 0051](docs/adr/0051-probability-scales.md)).
